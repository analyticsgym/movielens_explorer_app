...
def main():
    ...
    st.markdown(
        ...
        [MovieLens ml-latest-small documentation](https://files.grouplens.org/datasets/movielens/ml-latest-small-README.html)
        """
    )
    ...
    if selected_genre != "All":
        filtered_movies = movies[
            movies["genres"].str.contains(selected_genre, regex=False)
        ]
    else:
        filtered_movies = movies
    ...
    display_df = top_movies[
        ["title", "genres", "avg_rating", "rating_count", "IMDb Link"]
    ]

    st.dataframe(
        display_df,
        column_config={"IMDb Link": st.column_config.LinkColumn("IMDb Link")},
        hide_index=True,
    )
    ...
    selected_movie_title = st.selectbox(
        "Select a movie for recommendations", [""] + movie_titles
    )
        ...
        if not api_key:
            ...
            try:
                ...
                with st.spinner("Generating recommendations..."):
                    ...
                    formatted_recommendations = re.sub(
                        imdb_link_pattern,
                        replace_with_markdown_link,
    ...
    trailer_movie_title = st.selectbox(
        "Select a movie for trailer", [""] + movie_titles, key="trailer_movie_select"
    )
        ...
        with st.spinner("Finding trailer..."):
            ...
            try:
                ...
                if not api_key:
                    ...
                    trailer_link = response_2.output_text.strip()
                    if trailer_link:
                        st.session_state["trailer_link"] = trailer_link
                        st.session_state["trailer_movie"] = trailer_movie_title
            except Exception as e:
                st.error(f"Could not find trailer: {e}")

    # Display trailer from session state (only if it matches current selection)
    if (
        "trailer_link" in st.session_state
        and st.session_state.get("trailer_movie") == trailer_movie_title
        and st.session_state["trailer_link"]
    ):
